...
const JobCard: React.FC<JobCardProps> = ({ match }) => {
  const { state, dispatch } = useAppContext();
  const { brokenLinks, resumeText } = state;
  ...
  const handleGetReportCard = async () => {
    setIsReportModalOpen(true);
    if (reportCardData) return; // Don't re-fetch if already loaded
    
    setIsReportLoading(true);
    try {
      const result = await getResumeReportCard(resumeText, match);
      setReportCardData(result);
    } catch (error) {
      console.error("Failed to get resume report card:", error);
  ...
  const handleReportLink = () => {
    ...
    dispatch({ type: 'REPORT_BROKEN_LINK', payload: { url: match.jobUrl } });
  ...
  const isReported = brokenLinks.has(match.jobUrl);
  ...
  return (
    <>
          ...
          <div className="flex-1">
            <div className="flex flex-col sm:flex-row justify-between items-start gap-2">
              ...
              <div className='flex-shrink-0 flex items-center gap-2'>
                 <button 
                  onClick={handleGetReportCard}
                  className="px-3 py-2 bg-gray-700 text-amber-300 text-xs font-semibold rounded-md hover:bg-gray-600 transition-colors flex items-center gap-1.5"
                >
                  <ReportCardIcon />
                  Report Card
                </button>
            ...
            <div className="mt-4 pt-4 border-t border-gray-700 relative">
               <div className="absolute top-3 right-0">
                 <button
                  ...
                  disabled={isReported}
                  className="p-1 rounded-full text-gray-500 hover:bg-gray-700 hover:text-red-400 disabled:text-red-500 disabled:cursor-not-allowed"
                  title={isReported ? "Link reported as broken" : "Report broken link"}
                  aria-label={isReported ? "Link reported" : "Report broken link"}
                >
                  {isReported ? <CheckIcon /> : <BrokenLinkIcon />}
                </button>
                {showReportConfirmation && (
                  <div className="absolute top-full right-0 mt-2 bg-green-800 text-green-200 text-xs px-2 py-1 rounded-md shadow-lg whitespace-nowrap z-10">
                    Link reported. Thank you!
                  </div>
                )}
               </div>
      ...
      {/* Resume Report Card Modal */}
      <Modal title={`Resume Report Card for "${match.jobTitle}"`} isOpen={isReportModalOpen} onClose={() => setIsReportModalOpen(false)}>
        {isReportLoading ? (
          <Loader message="Generating Report Card..." />
        ) : reportCardData ? (
          <ResumeReportCardDisplay report={reportCardData} />
        ) : (
          <div className="text-center text-red-400">
            <p>Sorry, the report card could not be generated at this time.</p>
          </div>
        )}
      </Modal>
    </>
